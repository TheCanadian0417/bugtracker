...
.bt-quick { display:flex; gap:4px; margin-top:5px; }
.bt-quick button { font-family:'JetBrains Mono', monospace; font-size:10.5px;
  background:transparent; color:var(--fg3); border:1px solid var(--line);
  border-radius:2px; padding:2px 7px; cursor:pointer; }
.bt-quick button:hover { color:var(--fg); border-color:var(--line2); }
.bt-quick button[data-on="1"] { color:var(--wip); border-color:var(--wip); }
...
/* Local formatting. toISOString() converts to UTC first and can land you
   on the wrong day either side of midnight. */
const fmt = (d) =>
  `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;

/* Skips Saturday and Sunday. Company holidays are not accounted for. */
...
function Detail({ ticket, meta, isAdmin, log, onSave }) {
  const nextBiz = addBusinessDays(meta.today, 1);
  const [body, setBody] = useState("");
  const [status, setStatus] = useState(ticket.status);
  const [type, setType] = useState("Vendor response");
  const [vendorRef, setVendorRef] = useState(ticket.vendor_ref || "");
  /* A date already in the past is a date you've blown through — start from
     the next business day instead of re-offering it. */
  const [next, setNext] = useState(
    ticket.next_followup && ticket.next_followup >= meta.today ? ticket.next_followup : nextBiz
  );
  ...
  const submit = async () => {
    ...
    try {
      ...
      setBody("");
      setNext(addBusinessDays(meta.today, 1));
              ...
              {!closing && (
                <div className="bt-quick">
                  {quick.map(([label, date]) => (
                    <button key={label} type="button" data-on={next === date ? "1" : "0"}
                      onClick={() => setNext(date)}>{label}</button>
                  ))}
                </div>
              )}
            </div>
        ...
        <div className="bt-grid">
          <div className="bt-field">
            ...
            <input id="nt-opened" className="bt-in mono" type="date" max={meta.today}
              value={f.reportedDate} onChange={set("reportedDate")} />
          </div>
          <div className="bt-field">
